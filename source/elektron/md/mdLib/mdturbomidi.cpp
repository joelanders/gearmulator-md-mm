...
namespace md
{
		...
		const size_t _realtimeWriteBoundary)
	{
		...
		// Retire the previous allocation after releasing the short commit lock.
		std::vector<uint8_t> retired;
		{
			std::lock_guard lock(m_mutex);
			const auto state = m_state.load(std::memory_order_relaxed);
			if(state == MidiSysexTransferState::Queued
				|| state == MidiSysexTransferState::NegotiatingTurbo
				|| state == MidiSysexTransferState::Sending)
				return false;

			retired.swap(m_payload);
			...
			clearResponses();
			{
				const std::scoped_lock transmitLock(m_transmitMutex);
				m_transmitBytes.clear();
			}
			m_captureTransmit.store(false, std::memory_order_release);
			m_state.store(MidiSysexTransferState::Queued,
	...
	{
		if(!m_captureTransmit.load(std::memory_order_relaxed))
			return;
		const std::scoped_lock lock(m_transmitMutex);
		if(!m_transmitBytes.tryPush(_byte))
	...
	{
		// Elektron Machinedrum User's Manual, Appendix C: TurboMIDI protocol.
	...
	void TurboMidiTransfer::parseTransmitBytes()
	{
		...
		size_t byteCount = 0;
		{
			...
			while(byteCount < bytes.size()
				&& m_transmitBytes.tryPop(bytes[byteCount]))
				++byteCount;
		}
		...
		const bool _ingressDrained, MidiByteSink& _midiPort)
	{
		...
		case Phase::BeginNegotiation:
			m_captureTransmit.store(true, std::memory_order_release);
			queueMessage(0x10);
		...
		case Phase::WaitAnswer:
			if(takeResponse(0x11, response))
			{
				...
				if(certified & speed1Bit)
					m_speed2 = m_speed1;
				else
					m_speed2 = highestCode(common & (speed1Bit - 1u));
